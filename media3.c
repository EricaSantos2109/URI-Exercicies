...
mostre esta média acompanhada pela mensagem "Media: ". Se esta média for maior ou igual a 7.0,
...
int main()
{
    float n1, n2, n3, n4, mediaf, notaexame;
    scanf("%f %f %f %f", &n1, &n2, &n3, &n4);
    mediaf= ((n1 * 2) + (n2 * 3)+ (n3 * 4)+ (n4 * 1))/10;
    if(mediaf >= 7.0)
    {
    ...
    if(mediaf >= 5.0 && mediaf <= 6.9)
    {
        printf("Media: %.1f\nAluno em exame.\n", mediaf);
        scanf("%f", &notaexame);
        printf("Nota do exame: %.1f\n", notaexame);
			if (mediaf + notaexame / 2.0 >= 5)
			{
